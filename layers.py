...
class TemporalConvLayer(nn.Module):
    ...
    def forward(self, x):
        # x_in = self.align(x)[:, :, self.Kt - 1:, :] ## kt-1步预测，可以改成负数
        x_in = self.align(x)
        x_causal_conv = self.causal_conv(x)
        # print("x_in:", x_in.shape, "x_causal_conv:", x_causal_conv.shape)
        if x_causal_conv.size(2) > x_in.size(2):
            diff =  x_causal_conv.size(2) - x_in.size(2)
            pad = (0, 0, 0, diff)  # pad (last_dim, time_dim)
            x_in = F.pad(x_in, pad)
        ...
        if self.act_func == 'glu' or self.act_func == 'gtu':
            ...
            if self.act_func == 'glu':
                ...
                # print(x_p.shape, x_q.shape,x_in.shape)
