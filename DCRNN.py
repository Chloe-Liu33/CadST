...
class DCRNNModel(nn.Module, Seq2SeqAttrs):
    ...
    def forward(self, inputs, labels=None, batches_seen=None):
        ...
        :return: output: (self.horizon, batch_size, self.num_nodes * self.output_dim)
        """
        inputs = inputs.movedim(0,1).reshape(15,-1,inputs.size(2) * 2)
        if labels is not None:  #
            labels = labels.movedim(0,1).reshape(15, -1, labels.size(2) * 2)
        ...
        outputs = outputs.movedim(0,1).reshape(-1,15,self.num_nodes, 2)
        return outputs
...
class DCGRUCell(torch.nn.Module):
    ...
    @staticmethod
    def _build_sparse_matrix(L, sparsify_threshold=1e-3, verbose=True):

        if sparsify_threshold > 0:
            L = L.tocsr()
            L.data[np.abs(L.data) < sparsify_threshold] = 0
            L.eliminate_zeros()
        L = L.tocoo()
        ...
        if verbose:
            nnz = L_torch._nnz()
            density = nnz / (L.shape[0] * L.shape[1])
            print(f"[SparseMatrix] shape={L.shape}, nnz={nnz}, density={density:.6f}, device=cpu")
        L = torch.sparse_coo_tensor(indices.T, L.data, L.shape)
        # print(L.shape)
        L = L.coalesce().to(device)
        return L

    def forward(self, inputs, hx):
        """Gated recurrent unit (GRU) with Graph Convolution.
        :param inputs: (B, num_nodes * input_dim)ls
